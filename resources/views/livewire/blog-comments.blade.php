...
<div x-data="{ showComments: false, showForm: false }" class="my-4 md:my-8 p-3 md:p-6 rounded-lg shadow-lg bg-white dark:bg-zinc-900"
    x-init="window.addEventListener('hide-comment-form', () => { showForm = false })">
    <h3 class="text-2xl font-bold mb-3 md:mb-6 text-slate-800 dark:text-slate-100 text-center">Comments</h3>
    <div class="flex flex-col md:flex-row justify-center gap-4 mb-3 md:mb-6">
        <button @click="showComments = !showComments" class="text-sm flex items-center gap-2 px-5 py-2 hover:cursor-pointer rounded-full bg-slate-600 text-white hover:bg-slate-700 transition shadow">
        ...
        <button @click="showForm = !showForm" class=" text-sm hover:cursor-pointer flex items-center gap-2 px-5 py-2 rounded-full bg-blue-600 text-white hover:bg-blue-700 transition shadow">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
            <span x-show="showForm">Hide</span>
            <span x-show="!showForm">Add Comment</span>
        </button>
    </div>
    <div x-show="showComments" class="mb-4 md:mb-8">
        @forelse($comments as $comment)
            <div class="mb-3 md:mb-4 p-3 md:p-4 rounded-lg border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800 shadow">
                <div class="flex items-center justify-between mb-2">
                    <span class="font-semibold text-slate-700 dark:text-slate-200">{{ $comment->name }}</span>
                    <span class="text-xs text-gray-500 dark:text-gray-300">{{ $comment->created_at->format('M d, Y H:i') }}</span>
                </div>
                <div class="mt-2 text-gray-700 dark:text-gray-200">{{ $comment->comment }}</div>
            </div>
        @empty
            <div class="text-center text-gray-500 dark:text-gray-300">No comments yet.</div>
        @endforelse
    </div>
    <form x-show="showForm" wire:submit.prevent="submitComment" class="mx-auto max-w-lg bg-gray-50 dark:bg-zinc-800 p-3 md:p-6 rounded-lg shadow space-y-3 md:space-y-6">
        <div>
